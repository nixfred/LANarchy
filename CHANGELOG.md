...
All notable changes to LANarchy (`nixfred.lanarchy`) are documented here.
The version in `manifest.json` is the single source of truth.

LANarchy is a fork of [Lanarchy](https://github.com/DonnieFi/OmarPlugs) by Donnie
Fiander. Everything up to and including the `donnie-0.4.0` tag is shared history
with upstream; he merged five pull requests from this fork and released 0.4.0
...
- **Own plugin id: `nixfred.lanarchy`.** A hard fork that kept upstream's id could
  not be installed alongside it, and shipping divergent releases under someone
  else's id misrepresents who maintains them. Both can now coexist; existing
  installs of the upstream plugin are untouched
- **Named and credited honestly.** `manifest.json` carries this fork's maintainer
  and repository, and the README and this file credit Donnie Fiander as the
  original author, with `donnie-0.4.0` tagged here as the exact shared point
- **Everything upstream shipped in 0.4.0 is here**, because it came from this
  fork: runtime state out of the watched plugin tree, the probe gate that fails
  closed off the home network, MAC-anchored naming, OS identification from
  evidence already collected, the first-seen ledger, per-row map lanes so no wire
  crosses a card, measured-only motion, and the deduplicated health model
- **Seeding is consistent across every entry point.** `probe.py` and
  `inventory_cli.py` call `ensure_user_inventory()` rather than each resolving the
  starter their own way, so a one-shot probe seeds the same file the daemon does
- **The starter ships one node.** The `caddy-health` proxy example was a reverse
  proxy on a port most people do not run, and a card that is red on first launch
  reads as a broken install rather than an example
- Fixed: the List footer still named the LAN bucket card, which was removed in
  0.17.0, so it pointed at something the user could no longer see
...
- New `IpcHandler` (`nixfred.lanarchy`): `open`, `close`, `toggle`, `refresh`,
...
- Resolve plugin dir from install location (marketplace `nixfred.lanarchy`), not a hardcoded `homelab-mesh` path
